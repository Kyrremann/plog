...
function readAndPreview(form, preview, file) {
	...
	radio.type = 'radio';
	radio.name = 'feature_image';
	radio.value = file.name;
	radio.id = 'radio_' + file.name;
	radio.addEventListener('change', () => {
		const allRadios = document.querySelectorAll('input[name="feature_image"]');
		allRadios.forEach(r => {
			...
			try {
				...
				if (location) {
					...
					if (location.geocoding) {
						const locationName = `${location.geocoding.suburb}, ${location.geocoding.town}, ${location.geocoding.city}, ${location.geocoding.municipality}, ${location.geocoding.province}, ${location.geocoding.country}`;
		...
		fetch(url, {
			...
			.then(data => {
				if (data && data.address) {
					const { suburb, town, city, municipality, province, country } = data.address;
					resolve({ suburb, town, city, municipality, province, country });
				} else {
					resolve(null);
				}
			})
